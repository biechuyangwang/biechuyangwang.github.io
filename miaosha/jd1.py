...
webtools.get("https://www.jd.com/") # 打开淘宝或者京东
time.sleep(4)

# webtools.find_element_by_link_text("亲，请登录").click() # 准备扫码登录，这是旧版本的，需要使用新版本方法
webtools.find_element(By.LINK_TEXT, "你好，请登录").click()
...
while True:
    try:
        if webtools.find_element(By.NAME,"select-all"):
            webtools.find_element(By.NAME,"select-all").click()
            break
    except:
        print(f"找不到全选按钮")
...
while True:
    ...
    if now > pre_time:
        while True:
            try:
                if webtools.find_element(By.LINK_TEXT, "去结算"):
                    webtools.find_element(By.LINK_TEXT, "去结算").click()
                    print(f"已经锁定商品")
                    break
            except:
                pass
